			...
			<z-paging ref="paging" v-model="dataList" @query="queryList" :fixed="false" :safe-area-inset-bottom="true">
				<view slot="top" class="bg-white col w-100">
					...
					<view class="nav-wrap bg-white position-relative u-border-bottom" style="height:92rpx">
						<view class="position-absolute" style="left:0rpx;right:0rpx;z-index:999;">
							<view class="tabs-wrap">
								<u-tabs :list="tabsList" :current="tabsIndex" :is-scroll="false"
				...
				<view class="content py-1">
					<card-list-news v-if="tabsIndex===0" :dataList="dataList" />
					<card-list-partner v-if="tabsIndex===1" :dataList="dataList" />
					<card-list-project v-if="tabsIndex===2" :dataList="dataList" />
				</view>
	...
	export default {
		...
		methods: {
			...
			queryList(pageNo, pageSize) {
				...
				this.$api.myJoinList(type,pageNo,pageSize).then(res => {
			...
			// 跳转详情页面
			goProjectDetailPage() {
				this.$u.route('pages/mine/projectDetail')
			},
			// 跳转发布页面
			goPublishPage() {
