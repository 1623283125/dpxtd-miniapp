		...
		<view class="cart-wrap flex-1">
			<z-paging ref="paging" v-model="dataList" @query="queryList" :fixed="false" :safe-area-inset-bottom="true">
				...
				<view class="content py-1">
					<card-list-partner v-if="tabsIndex===0" :dataList="dataList" />
					<card-list-resource v-if="tabsIndex===1" :dataList="dataList" />
					<card-list-project v-if="tabsIndex===2" :dataList="dataList" />
					<card-list-funder v-if="tabsIndex===3" :dataList="dataList" />
					<card-list-news v-if="tabsIndex>=4" :dataList="dataList" />
				</view>
	...
	export default {
		...
		methods: {
			...
			queryList(pageNo, pageSize) {
				...
				this.$api.myPublishList(data,pageNo,pageSize).then(res => {
