	...
	<view v-if="info" class="info-page bg-white position-relative">
		...
		<view class="px-2">
			<view class="title py-2 row align-center justify-between">
				...
				<view class="font-12 radius-4 line-height-1 px-2 py-1" 
					@click="goWebsite(info.belongUnitWebsite)"
					style="background:#E4EAF7;color:#48A5FE;">
					进入官网
				</view>
			</view>
			<view class="pb-1 pl-15 py-1">
				<view class="title font-16 font-weight-500">
					{{info.title}}
				</view>
				<view class="content font-15 mt-2">
		...
		<view class="px-2 mt-2 u-border-top">
			<comment-list :tid="id" type="5" :collect="info.favoritesFlag" />
		</view>
...
<script>
	export default {
		...
		methods: {
			...
			// 获取详情数据
			getNewsInfo() {
				this.$api.getNewsInfo(this.id).then(res => {
