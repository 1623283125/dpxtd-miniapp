<template>
	<view class="htz-image-upload-list">
		...
		<view class="upload-list w-100" v-for="(item,index) in uploadLists" :key="index">
			
			<view v-if="item.type===2" class="row align-center font-12">
				<text class="">{{index+1}}.</text>
				<text class="flex-1 px-1">{{item.url.split('/').pop()}}</text>
				
				<view class="x-center tab-extend" @click="imgDel(index)">
					<u-icon name="close-circle-fill" color="red" size="32" />
				</view>
			</view>
		
			<view v-else class="htz-image-upload-Item">
				...
				<image v-else :src="getFileUrl(item)" @click="imgPreview(getFileUrl(item))" />
				<view v-if="remove" class="htz-image-upload-Item-del" @click="imgDel(index)">×</view>
			</view>
		
		</view>
		
		<view class="htz-image-upload-Item htz-image-upload-Item-add y-center" v-if="uploadLists.length<max && add" @click="chooseFile" style="background:#ebecee;border:0">
...
<script>
	export default {
		...
		data() {
			return {
				uploadLists: [],
				mediaTypeData: ['image', 'video', 'all', 'file'],
				previewVideoSrc: '',
			}
		},
		mounted: function() {
		...
		methods: {
			...
			FileAdd(){
				...
				let maxCount = (this.chooseNum > nowNum ? nowNum : this.chooseNum) //可选数量
				...
				wx.chooseMessageFile({
						...
						type: 'file',
						success: (res) => {
							let tempFilePaths = res.tempFiles.map(v=>v.path)
							this.chooseSuccessMethod(tempFilePaths, 2)
						},
				})
				// #endif
				// #ifdef H5
				...
				uni.chooseFile({
						...
						type: 'file',
						success: (res) => {
							this.chooseSuccessMethod(res.tempFilePaths, 2)
						},
				})
				// #endif
			},
			videoAdd() {
				...
				// #ifdef MP-WEIXIN
				// console.log('chooseMedia----',maxCount)
				// wx.chooseMedia({
				// 		count: maxCount,
				// 		mediaType: ['video'],
				// 		sourceType: this.sourceType,
				...
				// 		},
				// 		complete: (ret) => {
				// 			console.log('chooseMedia-complete---',ret)
				// 		}
				// });
				// #endif
						...
						const uploadTask = uni.uploadFile({
							...
							success: (uploadFileRes) => {
								...
								if (typeof this.uploadSuccess == 'function') {
									...
									let thisUploadSuccess = this.uploadSuccess(uploadFileRes)
