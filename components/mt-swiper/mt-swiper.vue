<template>
	<view class="u-swiper-wrap" :style="{
		...
		<swiper :current="elCurrent" @change="change" @animationfinish="animationfinish" :interval="interval" :circular="circular" :duration="duration" :autoplay="isAutoplay"
...
<script>
	...
	export default {
		...
		methods: {
			// 初始化信息
			initData(){
				let images = this.list.filter(v=>v.type==='image').map(v=>v.url)
				let videoLen = this.list.length - images.length
			...
			getSource(item) {
				...
				if (typeof item === 'object' && this.name) return item[this.name]
				else this.$u.toast('请按格式传递列表参数')
				return ''
			},
			// 预览图片
			btnViewImg(i){
